...
const fields = [
  ...
  { name: "size_spec", label: "Size / Specification" },
  { name: "material", label: "Material" },
  { name: "compatible_with", label: "Compatible With" }
];

export default function OthersInventory() {
  ...
  const loadCategories = async () => {
    try {
      const res = await axios.get(`/api/categories/others`);
  ...
  const load = async (q, cat) => {
    try{
      let url = `/api/inventory/others${q?`?q=${encodeURIComponent(q)}`:''}`
  ...
  const save = async (data) => {
    try{
      await axios.post(`/api/inventory/others`, data);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h3>Mechanical & Assembly Items</h3>
