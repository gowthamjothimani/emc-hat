...
const fields = [
  ...
  { name: "tool_type", label: "Tool Type" },
  { name: "brand", label: "Brand" },
  { name: "model_number", label: "Model Number" },
  { name: "serial_number", label: "Serial Number" },
  { name: "power_type", label: "Power Type" },
...
export default function ToolsInventory() {
  ...
  const loadCategories = async () => {
    try {
      const res = await axios.get(`/api/categories/tools`);
  ...
  const load = async (q, cat) => {
    try {
      let url = `/api/inventory/tools${q?`?q=${encodeURIComponent(q)}`:''}`
  ...
  const save = async (data) => {
    try {
      await axios.post(`/api/inventory/tools`, data);
