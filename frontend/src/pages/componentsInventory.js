...
const fields = [
  ...
  { name: "component_type", label: "Component Type" },
  { name: "part_number", label: "Part Number" },
  { name: "value_rating", label: "Value / Rating" },
  { name: "package_type", label: "Package Type" },
  { name: "tolerance", label: "Tolerance" },
  { name: "min_stock", label: "Minimum Stock Level", type: "number" }
];
...
export default function ComponentsInventory() {
  ...
  const loadCategories = async () => {
    try {
      const res = await axios.get(`/api/categories/components`);
  ...
  const load = async (q, cat) => {
    try{
      let url = `/api/inventory/components${q?`?q=${encodeURIComponent(q)}`:''}`
  ...
  const save = async (data) => {
    try{
      await axios.post(`/api/inventory/components`, data);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h3>Electronics Components Inventory</h3>
        <Button onClick={() => setShowAdd(true)}>+ Add Component</Button>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        <InputGroup style={{ maxWidth: 400 }}>
          <FormControl
            placeholder="Search by id/name/part..."
            value={search}
